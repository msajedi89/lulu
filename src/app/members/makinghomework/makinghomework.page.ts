...
export class MakinghomeworkPage implements OnInit {
  ...
  // **************** Inserting Exam ***************
  insertHomework(name, subject, homeworkDate) {
    ...
    if ((name != null) && (subject != null) && (this.myQuestionIDs != "") && (homeworkDate != null)) {
      ...
      this.network.insertHomework(name, subject, dateFormat, this.myQuestionIDs, this.myStudentIDs).then(data => {
        ...
        for (let i = 0; i < this.studentIDs.length; i++) {
          ...
          this.network.insertStudentHomework(myHomeworkID, stID).then(data => {
